...
def main():
    ...
    with st.container():
        ...
        with container_header[0]:
            ...
            with col_filter[0]:
                # st.text_input("Profit name", disabled=False)
                option = st.selectbox('Profit name', profit())
                # st.write('You selected:', option)
                st.text_input("Store code")

            with col_filter[1]:
                # st.text_input("Patch name", disabled=True)
                option = st.selectbox('Patch name', patch())
